...
const statusBadge = (studio: StudioRow) => {
  if (!studio.is_active && studio.reactivation_requested) {
...
const statusLabel = (studio: StudioRow) => {
  if (!studio.is_active && studio.reactivation_requested) return 'Pending Appeal';
  return studio.is_active ? 'Active' : 'Disabled';
};
...
export const StudiosManagement: React.FC = () => {
  ...
  const handleEditClick = (studio: StudioRow) => {
    ...
    setFormValues({
      ...
      bio: studio.bio || '',
      licence: studio.licence || '',
      street_address: studio.street_address || '',
      city: studio.city || '',
      state_province: studio.state_province || '',
      postal_code: studio.postal_code || '',
      country: studio.country || '',
      phone: studio.phone || '',
      email: studio.email || '',
      website: studio.website || '',
      social_twitter: studio.social_twitter || '',
      social_instagram: studio.social_instagram || '',
      is_active: studio.is_active !== undefined ? studio.is_active : true,
    });
    setMessage(null);
    setLicenceDocumentUrl(studio.licence_document_url || null);
    setViewMode('edit');
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formValues.stage_name.trim() || !formValues.bio.trim()) {
      setMessage({ type: 'error', text: 'Studio Name and Bio are required.' });
      return;
    }
    setIsSaving(true);
    setMessage(null);
    try {
      ...
      if (res.ok) {
        await fetchStudios();
        setMessage({ type: 'success', text: 'Studio details updated successfully!' });
        setTimeout(() => {
          setViewMode('list');
          setMessage(null);
        }, 1500);
        ...
        setMessage({ type: 'error', text: errorData.detail || 'Failed to update studio details.' });
      }
    } catch {
  ...
  const handleDisableStudio = async (studio: StudioRow) => {
  ...
  const handleEnableStudio = async (studio: StudioRow) => {
        ...
        <div className="space-y-6">
          <div className="flex flex-col sm:flex-row gap-4 bg-slate-900/10 border border-white/3 p-4 rounded-2xl shadow-inner text-xs items-center justify-between">
            <ListSearchInput
              ...
              placeholder="Search by studio, owner, or licence..."
            />
          ...
          <div className="hidden md:block overflow-x-auto rounded-3xl border border-white/5 bg-slate-900/10 backdrop-blur-md">
            {isLoading ? (
              <TableSkeleton rows={6} columns={6} variant="generic" />
            ) : filteredStudios.length === 0 ? (
              <p className="p-8 text-xs text-slate-500 text-center font-bold">No studios found.</p>
            ) : (
              <table className="w-full text-left border-collapse text-xs">
                ...
                <tbody className="divide-y divide-white/5">
                  {filteredStudios.map((studio) => (
                    <tr key={studio.id} className="hover:bg-slate-900/20 transition">
                      <td className="p-5">
                        <div className="font-bold text-slate-200">{studio.stage_name}</div>
                      </td>
                      <td className="p-5 text-slate-300">
                        {studio.city ? `${studio.city}, ${studio.country || ''}` : '—'}
                      </td>
                      <td className="p-5">
                        <div className="font-bold text-slate-200">{studio.owner_name || 'Unknown'}</div>
                        <div className="text-[10px] text-slate-455 mt-0.5">{studio.owner_email || '—'}</div>
                      </td>
                      <td className="p-5 max-w-[180px]">
                        <span className="text-slate-300 line-clamp-2">{studio.licence || '—'}</span>
                        <LicenceDocumentLink url={studio.licence_document_url} />
                      </td>
                      <td className="p-5">
                        <span className={`px-2 py-0.5 border rounded-full text-[9px] font-extrabold uppercase ${statusBadge(studio)}`}>
                          {statusLabel(studio)}
                        </span>
                      </td>
                      <td className="p-5">
                        <div className="flex items-center justify-center gap-2">
                          ...
                          {studio.is_active ? (
                            <button
                              type="button"
                              onClick={() => void handleDisableStudio(studio)}
                            ...
                            <button
                              type="button"
                              onClick={() => void handleEnableStudio(studio)}
                              className="p-2 bg-slate-900 border border-white/5 rounded-xl text-slate-400 hover:text-emerald-450 transition cursor-pointer"
                              title="Enable studio"
                            >
          ...
          <div className="md:hidden space-y-3">
            {isLoading ? (
              ...
              filteredStudios.map((studio) => (
                <div key={studio.id} className="rounded-2xl border border-white/5 bg-slate-900/20 p-4 space-y-3">
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      <div className="font-bold text-slate-200">{studio.stage_name}</div>
                    </div>
                    <span className={`px-2 py-0.5 border rounded-full text-[9px] font-extrabold uppercase flex-shrink-0 ${statusBadge(studio)}`}>
                      {statusLabel(studio)}
                    </span>
                  </div>
                  <div className="grid grid-cols-1 gap-2 text-[10px] text-slate-400">
                    <div><span className="text-slate-550 font-bold uppercase">Location: </span>{studio.city ? `${studio.city}, ${studio.country || ''}` : '—'}</div>
                    <div><span className="text-slate-550 font-bold uppercase">Owner: </span>{studio.owner_name || 'Unknown'} · {studio.owner_email || '—'}</div>
                    <div><span className="text-slate-550 font-bold uppercase">Licence: </span>{studio.licence || '—'} {studio.licence_document_url && (<a href={studio.licence_document_url} target="_blank" rel="noopener noreferrer" className="text-rose-400 ml-1">[Doc]</a>)}</div>
                  </div>
      ...
      {viewMode === 'edit' && (
        <form onSubmit={handleSubmit} className="bg-slate-900/10 border border-white/3 p-6 rounded-3xl shadow-inner space-y-6">
              ...
              <div className="glass-card p-6 rounded-3xl border border-white/5 space-y-4 shadow-xl">
                ...
                <div className="grid grid-cols-1 gap-4 text-xs">
                  <div className="space-y-1.5">
                    ...
                    <input type="text" name="stage_name" value={formValues.stage_name} onChange={handleInputChange} className="w-full bg-slate-950 border border-white/5 rounded-xl p-3 outline-none focus:border-rose-500 text-slate-200 transition" required />
                  </div>
                  <div className="space-y-1.5">
                    <label className="font-bold text-slate-400 uppercase tracking-wider block">Studio Bio *</label>
                    <textarea name="bio" value={formValues.bio} onChange={handleInputChange} rows={4} className="w-full bg-slate-950 border border-white/5 rounded-xl p-3 outline-none focus:border-rose-500 text-slate-200 transition resize-none" required />
                  </div>
                  <div className="space-y-1.5">
                  ...
                  {editingStudioId && (
                    <div className="space-y-1.5 sm:col-span-2">
                      ...
                      {licenceDocumentUrl ? (
                        ...
                        <p className="text-[10px] text-slate-500">No document uploaded by studio owner.</p>
                      )}
                    </div>
                  )}
                </div>
          ...
          <div className="flex justify-center gap-4 pt-2 flex-wrap">
            <button type="button" onClick={() => { setViewMode('list'); setMessage(null); }} className="px-8 py-3.5 bg-slate-900 hover:bg-slate-800 border border-white/5 text-slate-405 hover:text-white font-bold text-xs rounded-xl shadow-lg transition uppercase tracking-wider cursor-pointer">
              Cancel
            </button>
            {formValues.is_active ? (
              <button
                type="button"
                onClick={() => {
                  const current = studios.find((s) => s.id === editingStudioId);
                  if (current) void handleDisableStudio(current);
                }}
              ...
              <button
                ...
                onClick={() => {
                  const current = studios.find((s) => s.id === editingStudioId);
                  if (current) void handleEnableStudio(current);
                }}
            ...
            <button type="submit" disabled={isSaving} className="px-8 py-3.5 bg-rose-600 hover:bg-rose-500 disabled:opacity-50 text-white font-bold text-xs rounded-xl shadow-lg transition uppercase tracking-wider cursor-pointer">
              {isSaving ? 'Saving Details...' : 'Save Studio Details'}
            </button>
          </div>
        </form>
      )}
