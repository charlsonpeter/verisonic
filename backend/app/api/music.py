...
def queue_lyrics_extraction(
    ...
    from app.services.findlio_client import findlio_service_configured

    sync_mode = bool(lyrics_text and lyrics_text.strip())

    try:
        validate_pipeline_config(settings)
        # Remote Findlio holds Google credentials; skip local sync prechecks when configured.
        if sync_mode and not findlio_service_configured():
            validate_sync_pipeline_config(
